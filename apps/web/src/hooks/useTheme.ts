...
const LEGACY_STORAGE_KEYS = ["synara:theme"] as const;
...
function readStoredThemeStateFromStorage(): ThemeState {
  if (!hasThemeStorage()) {
  ...
  if (rawValue !== null) {
    return parseStoredThemeState(rawValue);
  }

  for (const legacyStorageKey of LEGACY_STORAGE_KEYS) {
    const legacyRawValue = readStorageValue(legacyStorageKey);
    if (legacyRawValue !== null) {
      return parseStoredThemeState(legacyRawValue);
    }
  }
...
function logThemeStorageReadFailure(cause: unknown): ThemeStorageError {
...
function readStoredThemeState(): ThemeState {
  ...
  try {
    return readStoredThemeStateFromStorage();
  } catch (cause) {
    logThemeStorageReadFailure(cause);
    return DEFAULT_THEME_STATE;
  }
}
...
export function readThemePreference(): ThemeMode {
  return readStoredThemeStateFromStorage().mode;
}
...
function subscribe(listener: () => void): () => void {
  ...
  const handleStorage = (event: StorageEvent) => {
    if (
      ...
      !LEGACY_STORAGE_KEYS.some((legacyStorageKey) => legacyStorageKey === event.key)
    ) {
      return;
    }
    themeStorageReadFailure = null;
